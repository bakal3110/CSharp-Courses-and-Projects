...
// Reads directional input from the Console and moves the player
void Move(bool exitGameWithAnyKey = false, bool IncreasedMovementSpeed = false)
{
    int lastX = playerX;
    int lastY = playerY; 
    ...
    switch (Console.ReadKey(true).Key) 
    {
		...
		case ConsoleKey.DownArrow: 
            ...
            break;
		case ConsoleKey.LeftArrow:  
            playerX = (IncreasedMovementSpeed && player == state[1]) ? (playerX - 4) : playerX--; 
            break;
		case ConsoleKey.RightArrow: 
            playerX = (IncreasedMovementSpeed && player == state[1]) ? (playerX + 4) : playerX++; 
            break;
